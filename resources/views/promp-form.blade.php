...
<body class="p-3 h-full">
    ...
    <section class="w-3xl p-3 mx-auto border border-cyan-300 mt-3 rounded-sm shadow-sm">
        ...
        <div class="text-center mb-3">
            <h4 class="text-2xl">GPT Prompt</h4>
        </div>
        {{-- form  --}}
        <div>
            <form action="{{ route('chat') }}" method="POST" enctype="multipart/form-data" id ="prompt-form">
                ...
                <div class="w-full mb-4">
                    <label for="prompt" class="block text-gray-700 text-sm font-bold mb-2">Write Service
                        Prompt</label>
                    <textarea class="focus:outline-0 block w-full px-2 py-3 rounded-sm shadow-sm shadow-cyan-300" id="prompt"
                        name="prompt" placeholder="prompt goes here..."></textarea>
                </div>

                {{-- file upload  --}}
                <div class="w-full mb-4">
                    <label for="prompt" class="block text-gray-700 text-sm font-bold mb-2">Upload Logo</label>
                    <input type="file" class="block w-full px-2 py-3 rounded-sm shadow-sm" id="logo"
                        name="logo" name="prompt" placeholder="prompt goes here..." />
                </div>

                {{-- submit button  --}}
                <div class="w-full mb-4">
                    <input type="submit" value="Generate"
                        class="bg-blue-500 hover:bg-blue-500 text-white  py-2 px-4 rounded focus:outline-none focus:shadow-outline block w-full" />
            </form>
            ...
            @if (session('message'))
                <div class="border border-cyan-300 p-3 rounded-sm">
                        ...
                        <div>
                            @if (session('logo_url'))
                                <img src="{{ session('logo_url') }}" alt="Logo"
                                    class="w-20 h-20 rounded-full mx-auto mb-4 object-fill">
                            @endif
                        </div>
                    ...
                    {{-- download button  --}}
                    <a href="{{ route('pdf', ['chat' => session('promptId') ? session('promptId') : '']) }}"
                        class="shadow-sm shadow-cyan-300 py-2 px-3 rounded-sm">Download File</a>
                </div>
            @endif
        </div>
    </section>
